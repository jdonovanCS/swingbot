...
# Function to retrieve all symbols from MT5
def get_all_mt5_symbols():
    ...
    symbols = []
    try:
        symbols = [symbol.name for symbol in MetaTrader5.symbols_get()]
        print(len(symbols))
    except Exception as e:
        print("Symbols not able to be retrieved from mt5. {e}")
...
def place_order(order_type, symbol, volume, stop_loss, take_profit, comment, stop_price, direct=False):
    ...
    if direct:
        ...
        if order_result[0] == 10009:
            ...
            print(f"order_result in place order: {order_result[2]}")
        ...
        # Notify the user if AutoTrading has been left on in MetaTrader 5
            ...
            if tick is None:
                ...
                # Calculate the current price as the average of bid and ask
                print(f"current ask price: {tisk.ask}")
            print(f"Try playing with deviation in make_trade.py")
        elif order_result[0] == 10021:
            print(f"No prices for {symbol}. Price: {stop_price}")
            tick = MetaTrader5.symbol_info_tick(symbol)
            if tick is None:
                print(f"Failed to get tick data for {symbol}.")
            else:
        ...
        if tick is None:
            ...
            # Calculate the current price as the average of bid and ask
            print(f"ask price: {tick.ask}, bid price: {tick.bid}")
        ...
        # Let user know if an invalid price has been passed
            ...
            if tick is None:
                ...
                if request['type'] == MetaTrader5.ORDER_TYPE_BUY:
                # Calculate the current price as the average of bid and ask
                    print(f"buying... current ask price: {tick.ask}")
                elif request['type'] == MetaTrader5.ORDER_TYPE_SELL:
                    print((f"selling... current bid price: {tick.bid}"))
            print(f"Try adjusting the deviation in make_trade.py")
        elif result[0] == 10021:
            print(f"No prices for {symbol}. Price: {stop_price}")
            tick = MetaTrader5.symbol_info_tick(symbol)
            if tick is None:
...
def cancel_order(order_number, symbol=None):
    """
...
def cancel_filtered_orders(symbol, comment):
    ...
    if len(orders) > 0:
        ...
        for order in orders:
            cancel_outcome = cancel_order(order, symbol)
...
# Function to retrieve a filtered list of positions MT5
def get_filtered_list_of_positions(symbol, comment):
    """
    ...
    """
    # Retrieve a list of open orders, filtered by symbol
    positions_by_symbol = MetaTrader5.positions_get(symbol)
    # Check if any positions were retrieved (there may be none)
    if positions_by_symbol is None or len(positions_by_symbol) == 0:
        return []

    # Convert the retrieved orders into a dataframe
    positions_dataframe = pandas.DataFrame(
        list(positions_by_symbol),
        columns=positions_by_symbol[0]._asdict().keys()
    )
    # From the open orders dataframe, filter orders by comment
    positions_dataframe = positions_dataframe[positions_dataframe['comment'] == comment]
    # Create a list to store the open order numbers
    positions = []
    # Iterate through the dataframe and add order numbers to the list
...
def close_filtered_positions(symbol, pos_type, comment):
    ...
    """
    # Retreive a list of the positions based upon the filter
    positions = get_filtered_list_of_positions(
        symbol=symbol,
        comment=comment
    )
    if len(positions) > 0:
