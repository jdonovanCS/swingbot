...
def calc_lot_size(balance, risk_amount, stop_loss, stop_price, symbol):
    ...
    pip_size = MetaTrader5.symbol_info(symbol).trade_tick_size*10
...
# Add in another counter currency example
    ...
    print(f"Lot values:\npip_size: {pip_size}, stop_pips: {stop_pips_integer}, pip_value: {pip_value}, raw_lot_size: {raw_lot_size}, lot_size: {lot_size}")
    if lot_size >= 10:
        lot_size = .99
    return lot_size
